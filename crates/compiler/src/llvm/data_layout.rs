...
impl DataLayout {
    ...
    pub fn new(_layout_string: &str) -> Result<Self> {
        // Self::parser()
        //     .parse(layout_string)
        //     .map_err(|e| Error::InvalidDataLayoutSpecification(layout_string.into()))
        unimplemented!()
    }

    // /// Builds a parser for the data-layout specification.
    // fn parser() -> impl Parser<char, DataLayout, Error = Simple<char>> {
    //     unimplemented!()
    // }
}
    ...
    pub fn parser() -> impl parsing::DLParser<IntegerLayout> {
        just("i")
            ...
            .try_map(|((size, abi_alignment), preferred_alignment), span| {
                let preferred_alignment = preferred_alignment.unwrap_or(abi_alignment);
                if size == BYTE_SIZE as usize && abi_alignment != size {
                    Err(Simple::custom(
                        span,
    ...
    pub fn stack_alignment() -> impl DLParser<usize> {
        just("S")
            ...
            .validate(|alignment, span, emit| {
                if alignment % BYTE_SIZE as usize != 0 {
                    emit(Simple::custom(
                        span,
