...
//! - **Transformations:** Transformation passes use either information from
...
impl dyn PassOps {
    ...
    pub fn unwrap_as<T: ConcretePass>(&self) -> &T {
        self.view_as::<T>()
            .expect(&format!("self was not a {:?}", TypeId::of::<T>()))
    }
    ...
    Self: Clone + Debug + PassOps,
{
    ...
    /// Gets a key that uniquely represents the pass.
    ///
    /// This **must** return the same value as [`PassOps::dyn_key`].
    fn key() -> PassKey {
        TypeId::of::<Self>()
    }
}
...
impl PassManager {
    ...
    pub fn run(&mut self, mut context: SourceContext) -> Result<PassManagerReturnData> {
        ...
        for pass in self.pass_ordering.iter_mut() {
            let PassReturnData {
                source_context,
                data,
