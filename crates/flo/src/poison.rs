...
/// Trait for FLO values that can be considered poisoned.
pub trait Poisonable {
    /// Returns a poison value for a T with the given PoisonType.
    fn get_poison_value(typ: PoisonType) -> Self;

    /// Returns whether or not the given value is poisoned.
    fn is_poisoned(value: &Self) -> bool;
}

// Helpers for generating poisonable types.
