//! In-memory representation of a FlatLoweredObject, as used for working with
//! FLOs.
//!
...
use crate::{intern::InternTable, types::*};
...
pub struct TypeTables {
    ...
    pub array: InternTable<ArrayTypeId, ArrayType>,

    /// Stores the definitions of our struct Types.
    pub struc: InternTable<StructTypeId, StructType>,
}
...
impl TypeTables {
    ...
    pub(crate) fn new() -> Self {
        Self {
            array: InternTable::new(),
            struc: InternTable::new(),
        }
    }
}

/// The core in-memory representation of a FLO file.
/// Used for building, linking, and mutating FlatLoweredObject.
#[derive(Serialize, Deserialize, Debug)]
pub struct FlatLoweredObject {
    ...
    /// lifetime. Most variables are anonymous -- they have no symbol name,
    /// and may or may not contain a human-friendly name in their diagnostic
    /// data.
    ...
    /// Stores any diagnostic messages associated with any of the above types.
    pub diagnostics: InternTable<DiagnosticId, Diagnostic>,

    /// Stores any locations associated with any of the above types.
    ...
    /// If set, this allows loading or emitting of files that contain poison
    /// values in referenced places. Can be used to allow objects to be
    /// serialized mid-construction.
...
impl FlatLoweredObject {
    ...
    /// completed before emission. This is probably not what you want; but
    ...
    /// Creates a new FlatLoweredObject representation from a string
    /// representation; for example, from a string read from a .flo file.
    pub fn from_str(encoded: &str) -> serde_sexpr::Result<Self> {
