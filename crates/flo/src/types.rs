...
/// of [Statement]s and terminating in a [BlockExit]. All of a program's
/// real work -- i.e. everything but contorl flow -- is accomplished in the
/// sequence of statements that make up a block's body.
...
pub struct Block {
    ...
    /// Indicates whether this block is a _poison value_. This is typically
...
/// A reference to an object of type [Block] in one of the [FlatLoweredObject]'s
/// interning tables.
pub type BlockId = InternIdentifier;

/// A Signature contains the metadata necessary to call a
/// f[FlatLoweredObject]/// Signatures are stored within the relevant [Block],
/// and thus are currently not interned.
...
/// Indicates whether a containing object is a _poison value_,
/// and this should not be included in reasonable use.
#[derive(Clone, Serialize, Deserialize, Debug, Default)]
pub enum PoisonType {
    ...
    /// Indicates that the value has been manually poisoned,
    /// along with an informational message regarding why.
    Poison(String),

    /// Indicates that this entry refers to an object that is declared,
    /// but not yet defined.
    Undefined,

    /// Indicates a block that never should be reached; or a variable
    /// that should not ever be accessed.
    Unreachable,

    // Special case: this is the value of the Null interned entry,
    // which sits at 0 and tries to prevent logic bugs.
    NullInternedValue,
}
...
/// Determines the action to be taken once the execution of a [Block]
/// is complete.
#[derive(Clone, Serialize, Deserialize, Debug, Default)]
pub enum BlockExit {
    /// Indicates that control flow should return to the caller.
    /// Should be provided with a list of variable IDs for the function
    /// to return.
    ...
    /// Indicates that we should iterate through a list of [Matcher]s,
    /// and continue to the [Block] associated with the first mathcing
    /// predicate.
    Match(Vec<MatchArmId>),

    /// For internal use -- indicates that this BlockExit is unspecified,
    /// e.g. as part of a poisoned [Block]. You should not set this unless
    /// also poisoning the block this is contained in; encountering it during
    /// object code emission is a compile-time error.
    #[default]
    Unspecified,
}

/// Describes a single step in program execution.
/// See the *Statement types for more information.
#[derive(Clone, Serialize, Deserialize, Debug)]
pub enum Statement {
...
/// [FlatLoweredObject]'s interning tables.
...
pub struct CallStatement {
    ...
    /// The SSeThe IR prefix call.
    /// Effectirtype arguments to the call.
...
/// The type of the constructed obeject is determined by the type of
/// [Variable] is to be bound to ("target").
#[derive(Clone, Serialize, Deserialize, Debug)]
pub struct ConstructStatement {
    /// The variable the constructed object is to be bound to.
    /// DetermineThe IR prefix construct operation.
    ...
    pub initilaizer: Vec<VariableId>,
...
pub struct DestructureStatement {
    ...
    /// The variables to be made equivalent to the components of the
    /// composite type, in order.
...
/// "Dereferences" a snapshot, binding a new variable to the value
...
pub struct DesnapStatement {
    ...
    /// The variable to be 'populated' with the value of the snapshot'd
...
pub enum BlockRef {
    ...
    ///
    /// NOTE: The symbol name provided must be mangled as required by the
    /// relevant language.
    ///
    /// NOTE: Symbol names starting with two or more underscores are reserved
    /// for the implementation.
    External(String),

    /// Points to a symbol located in the implementation's runtime library
    /// (the "CompilerRT") in order to perform a runtime operation on behalf of
    /// the compiler.
    Builtin(String),

    /// For internal use -- indicates that this BlockRef's target is
    /// unspecified, e.g. as part of a poisoned [Variable]. You should not
    /// set this unless also poisoning the object this is contained in;
    /// encountering it during object code emission is a compile-time error.
    #[default]
    Unspecified,
}

/// Represents an SSA variable -- simple or composite, and provides
/// basic metadata about that variable.
#[derive(Clone, Serialize, Deserialize, Debug, Default)]
pub struct Variable {
...
/// Specifies the location of a given variable's storage --
/// e.[FlatLoweredObject]r the variable is local, or in another translation
/// unit.
#[derive(Clone, Serialize, Deserialize, Debug, Default)]
pub enum VariableLinkage {
    ...
    /// Indicates that the variabe exists in another translation unit,
    /// and shold be resolved by the provided symbol.
    External(String),
    ...
    /// unspecified, e.g. as part of a poisoned [Variable]. You should not
    /// set this unless also poisoning the variable this is contained in;
    /// encountering it during object code emission is a compile-time error.
    #[default]
    Unspecified,
}

/// Specifies the simple or composite type of a given SSA variable.
...
pub enum Type {
    ...
    /// For internal use -- indicates that this Variable's type is unspecified,
    /// e.g. as part of a poisoned [Variable]. You should not set this unless
    /// also poisoning the variable this is contained in; encountering it during
    /// object code emission is a compile-time error.
    #[default]
    Unspecified,
}
...
/// Represents an array of types that can be used with
#[derive(Clone, Serialize, Deserialize, Debug, Default)]
...
/// access their individual parts. To support languages with strict typing,
/// CompositeTypeIds can be treated as unique type identifiers, and compared.
#[derive(Clone, Serialize, Deserialize, Debug, Default)]
...
/// Contains a simple string message that can be associated with a
/// given object; as well as an optional associated [Location].
#[derive(Clone, Serialize, Deserialize, Debug, Default)]
...
/// Contains a simple "pointer" to a piece of source material, for
...
pub struct Location {
    ...
    /// The line number in the relevant source file, if
    /// availabl[FlatLoweredObject]evant.
...
/// [FlatLoweredObject] of the relevant match condition has already been
/// evaluated and stoed in a [Variable] of type Bool. Directs control flow to
/// the provided Block if the relevant [Variable] is true.
#[derive(Clone, Serialize, Deserialize, Debug, Default)]
pub struct MatchArm {
    ...
    /// Indicates whether this arm is a _poison value_. This is typically
    /// None, indicating that this value is unpoisoned.
    pub poison: PoisonType,
